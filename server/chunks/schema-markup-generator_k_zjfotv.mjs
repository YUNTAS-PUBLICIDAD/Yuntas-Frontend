function insertJsonLd(type, mapped) {
  try {
    let jsonLd;
    if (type === "product") {
...
function generateBlogJsonLd(blog) {
  ...
  const description = blog.etiqueta?.meta_descripcion || blog.subtitulo || blog.parrafos?.map((p) => p.parrafo).join(" ");
  return {
    "@context": "https://schema.org/",
    "@type": "Blog",
