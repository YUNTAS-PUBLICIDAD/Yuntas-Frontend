...
const textosBoton = ["Explorar opciones"];
...
const Emergente_Producto = ({ producto }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl(config.endpoints.productos.info), {
        method: "POST",
        headers: {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center px-4">
      ...
      >
        ...
        {/* Formulario */}
        <div className="w-full sm:w-[40%] pr-1 pt-10 pb-10 pl-1 relative flex flex-col justify-center">
          <button
            onClick={closeModal}
            aria-label="Cerrar modal"
          ...
          {/* Contenedor reducido */}
          <div className="w-full text-center max-w-[250px] mx-auto">
            <div className="mb-4 sm:mb-6">
              <h2
                className="text-lg sm:text-3xl font-extrabold font-montserrat leading-tight text-center pt-2 pb-6"
                style={{ color: "#0E3F88" }}
              >
                {textoData.titulo}
              </h2>
            ...
            <form className="space-y-3 flex flex-col items-center" onSubmit={handleSubmit}>
              <input
                type="text"
                ...
                className={`w-[90%] mx-auto px-4 py-1 rounded-lg bg-[#EBEBEB] border-0 text-sm font-montserrat focus:ring-2 ${
                  errors.nombre
                    ? "ring-2 ring-red-400"
...
export default Emergente_Producto;
